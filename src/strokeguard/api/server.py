...
@app.get("/health")
def health(): return {"status":"ok","mode":settings.mode,"ai_local":True}
...
def sensor():
    ...
    inference.push(p); pred=inference.predict(); decision=fusion.decide(p,pred)
    return {"sensor":p.to_dict(),
