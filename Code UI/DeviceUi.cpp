...
void DeviceUi::showWarning(const VitalSigns& vitals, uint8_t countdownSeconds) {
  ...
  drawCenteredText("WARNING", 48, 3, COLOR_WARNING);
  drawRiskRing(vitals.riskScore, COLOR_WARNING);

  tft.setTextColor(COLOR_TEXT, COLOR_BG);
  tft.drawCentreString("Abnormal BP/HR", SCREEN_W / 2, 158, 2);
  tft.drawCentreString("Are you OK?", SCREEN_W / 2, 181, 2);

  tft.setTextColor(COLOR_TEXT, COLOR_BG);
  tft.drawCentreString("SOS in " + String(countdownSeconds) + "s", SCREEN_W / 2, 207, 2);

  drawSoftButton("I'M OK", COLOR_SAFE);
}
